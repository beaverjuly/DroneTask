...
jsPsych.plugins['comprehension'] = (function() {
  ...
  function renderQuestion(prompt, idx, state) {
    ...
    if (status === 'correct-locked') {
      ...
      statusEl = '<span class="q-status" style="color:#d97706;">Change your answer</span>';
  ...
  function renderAll(state, justReviewedTopic) {
    ...
    html += '<button type="button" id="comp-submit" class="' +
              (btnDisabled ? 'disabled' : '') + '">Check answers</button>';
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var allComp = jsPsych.data.get().filter({ trial_type: 'comprehension' }).values();
    ...
    function onSubmitClick() {
      ...
      if (!isAllCorrect(state)) {
        showToast('Some answers are still wrong. Review instructions and change them.', 'warn');
        return;
      }
